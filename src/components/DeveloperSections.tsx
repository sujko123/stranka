...
export function JourneySection() {
  ...
  return <section id="timeline" className="dev-section">
    ...
    <Reveal className="dev-panel journey-panel">
      {tab === 0 && <div className="education-list"><article><GraduationCap /><div><span className="small-mono text-primary">2026 — {c("súčasnosť", "present")}</span><h3>VŠE · Business Informatics</h3><span className="education-sub">{c("Vysoká škola ekonomická v Prahe · Ing.", "Prague University of Economics and Business · Master's degree")}</span><p>{c("Magisterské štúdium zamerané na prepájanie IT a biznisových potrieb. Nadväzuje na témy bakalárskeho štúdia z oblasti informačných systémov, dát a riadenia IT projektov.", "A master's programme focused on connecting IT with business needs. It builds on my bachelor's studies in information systems, data and IT project management.")}</p></div></article><article><GraduationCap /><div><span className="small-mono">2023 — 2026</span><h3>VUT · {c("Manažérska informatika", "Managerial Informatics")}</h3><span className="education-sub">{c("Fakulta podnikateľská, Brno · Bc.", "Faculty of Business and Management, Brno · Bc.")}</span><p>{c("Analýza a návrh informačných systémov, UML a ER diagramy, relačné databázy a SQL, dátové modelovanie, dátové sklady, OLAP, Power BI, Excel, VBA, analýza podnikových procesov a projektový manažment.", "Information systems analysis and design, UML and ER diagrams, relational databases and SQL, data modelling, data warehouses, OLAP, Power BI, Excel, VBA, business process analysis and project management.")}</p></div></article><article><GraduationCap /><div><span className="small-mono">2016 — 2022</span><h3>C. S. Lewis</h3><span className="education-sub">{c("Anglické gymnázium, Bratislava", "English grammar school, Bratislava")}</span><p>{c("Maturita z anglického jazyka na úrovni C1.", "English graduation exam at C1 level.")}</p></div></article></div>}
...
`, "Swimming was a big part of my childhood.Veľkú časť môjho detstva tvorilo plávanie. Postupne sa z neho stal vrcholový šport a päť rokov som bol súčasťou slovenskej reprezentácie. Naučilo ma disciplíne, vytrvalosti a tomu, že dobré výsledky neprichádzajú zo dňa na deň. Aj keď už dnes netrávim väčšinu času v bazéne, tento prístup si prenášam do všetkého, čo robím. K technológiám som sa dostal ešte na strednej škole cez súťaže v Pythone. Neskôr som vytvoril web pre rodinný projekt, pri ktorom som objavil JavaScript a svet webového vývoja. Z jedného projektu sa postupne stala freelance práca a dnes mám za sebou viac ako 30 webových stránok a aplikácií pre klientov aj vlastné nápady. Baví ma prepájať technickú a biznisovú stránku projektu. Nejde mi iba o to, aby web dobre vyzeral. Chcem rozumieť tomu, komu má slúžiť, aký problém rieši a akú hodnotu má priniesť. Neustále sa posúvam v Reacte, TypeScripte, Next.js a backende, pričom nové veci si najradšej overujem priamo v praxi.")}</p><div className="journey-path"><span>Python</span><span>→</span><span>Frontend</span><span>→</span><span className="text-secondary">Full-stack</span></div></div>}
      {tab === 2 && <div className="journey-story"><Trophy className="text-secondary" size={32} /><h3>{c("Čo robím, keď neprogramujem?", "What do I do when I'm not programming?")}</h3><p>{c("Vo voľnom čase rád cvičím, pozriem si dobrý film, zahrám si šach na chess.com alebo sa venujem investovaniu a financiám. A keď sa vrátim k obrazovke, často je to kvôli novému UI/UX nápadu vo Figme.", "In my free time, I enjoy working out, a good film, a game of chess on chess.com, or learning about investing and finance. When I return to the screen, it's often to explore a new UI/UX idea in Figma.")}</p><div className="offline-tags"><span>♞ {c("šach", "chess")}</span><span>↗ {c("cvičenie", "fitness")}</span><span>▷ {c("filmy", "films")}</span><span>€ {c("investovanie / financie", "investing / finance")}</span><span>⌘ UI / UX</span></div><span className="small-mono text-muted-foreground">// There is no place like 127.0.0.1</span></div>}
    </Reveal>
  </section>;
}
